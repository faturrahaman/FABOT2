 ...
 if (!isOwner) return reply(mess.only.ownerB)
            ...
            break
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchJson(`https://api.vhtear.com/primbonjodoh?nama=${gbl11}&pasangan=${gbl21}&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					pinte = body.slice(11)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=${pinte}&apikey={BELI APIKEY BIAR WORK DI 0816546638}`, {method: 'get'})
					reply(mess.wait)
